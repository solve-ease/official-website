...
// Import CodeMirror components for markdown editing
...
useEffect(() => {
  ...
  useEffect(() => {
    if (editorView && formData.content && !editorView.state.doc.toString()) {
      editorView.dispatch({
        changes: { from: 0, to: editorView.state.doc.length, insert: formData.content }
      });
    }
  }, [editorView, formData.content]);
  ...
  const MarkdownToolbar = () => {
    return (
      <div className="border-b border-gray-200 pb-2 mb-4 flex flex-wrap gap-1">
        ...
        <button
          onClick={() => insertMarkdown('image', 'image alt text')}
          className="px-2 py-1 rounded bg-white hover:bg-gray-100"
          type="button"
        >
          Image
        ...
        <button
          ...
          className={`px-2 py-1 rounded ${markdownPreview ? 'bg-gray-200' : 'bg-white hover:bg-gray-100'}`}
          type="button"
        >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <div className="border border-gray-300 rounded-md overflow-hidden">
            ...
            {markdownPreview ? (
              <div className="min-h-[400px] prose max-w-none p-4 bg-white">
                <ReactMarkdown>
                  {formData.content}
                </ReactMarkdown>
              </div>
